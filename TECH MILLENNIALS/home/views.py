...
def telegram(request):
    ...
    if request.method == 'POST':
      channel_username = request.POST.get('video_url')
      ...
      api_key = "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"

      # Description for which you want an explanation
      prompt=channel_username+"/n"+"I am doing a project to classify misleading finfluencers in social media. I gave you some chat texts from a telegram channel which is providing financial content. analyze the chat text. tell what he is doing. check for any misleading claims like giving calls on forex market in unregistered app, crypto currency meme coins, crypto trading with high volatality, Pump and dump schemes, money doubling schemes, instant loan schems or any misleading financial activities. If he also blindly giving trading calls on stocks options or futures without any proper research ad technical analysis, detect that also. so see for misleading or illegal claim. give your inference in paragraph"
...
def youtube(request):
    ...
    if request.method == 'POST':
      ...
      def retrive(VIDEO_URL):
        ...
        return video_id, channel_id, outls, comments, channel_title, channel_description, video_title, video_description
      
      video_id, channel_id, outls, comments, channel_title, channel_description, video_title, video_description = retrive(video_url)  
      ...
      if predicted_description==1:
        description_score=class_probs_description[1]
      else:  
        description_score=class_probs_description[0]
      print()
      ...
      print("Comments")
      print(comment_pred)
      if comment_pred.count(1)>comment_pred.count(0):
         print("Legit")
      else:
         print("Not legit") 
      comments_score=comment_pred.count(1)/100    
      ...
      import openai
      # Set your OpenAI GPT-3 API key
      api_key = "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"
      # Generate explanation using GPT-3 API
      prompt=video_description+"/n"+"I gave you the description of the youtube video related to finance which is highly misleading . he promises high returns dicussing option strategies in quotex or binomo app thet is highly misleading what is your view?"  
      openai.api_key = api_key
      response = openai.Completion.create(
          engine="text-davinci-003",
          prompt=prompt,
      ...
      analysis_result = AnalysisResult(
            ...
            promotion=promotion,
            overall_score=description_score,
